...
model = SemanticInstanceSegmentation(5).cuda()
model.load_state_dict(
    torch.load('C:\\Users\\sichu\\dev\\PyCharm-Workspace\\semantic-segmentation-master\\models\\epoch_40'))
model.eval()

data_unlabelled = ImageFolder(
    root='C:\\Users\\sichu\\dev\\PyCharm-Workspace\\semantic-segmentation-master\\data\\wyoming', transform=ToTensor())
loader = torch.utils.data.DataLoader(data_unlabelled, batch_size=1, shuffle=True)

# Variable(next(iter(loader))).cuda(), (0, 0, 3, 3)
image = F.pad(Variable(next(iter(loader))).cuda(), (0, 0, 3, 3))
_, logits, instance_embeddings = model.forward_clean(image)

# del (logits, instance_embeddings, image)
